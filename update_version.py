...
if not re.match(semver_regex, arg_current_version):
    print(f"Error: Current version '{arg_current_version}' does not match the Semantic Versioning pattern.")
    sys.exit(1)

if not re.match(semver_regex, arg_new_version):
    print(f"Error: New version '{arg_new_version}' does not match the Semantic Versioning pattern.")
    sys.exit(1)

# Abort if both versions are identical.
if arg_current_version == arg_new_version:
    print(f"Error: New version '{arg_new_version}' equals the current version '{arg_current_version}' (nothing to do here).")
    sys.exit(1)
...
@contextmanager
def update_version_in_file(file_path: str, current_version: str, new_version: str) -> None:
    # Read the file.
...
def update_versions(file_paths: List[str], current_version: str, new_version: str) -> None:
